...
function UserBooking() {
    ...
    useEffect(()=>{
        const fetchData = async ()=>{
            const bookingData = await axios.get(`${REACT_APP_ENDPOINT}/tablebooking`);
            console.log(bookingData.data.data);
            setUserData(bookingData.data.data);
        }
        fetchData();
    },[])
    // useEffect(() => {
    //     const fetchData = async () => {
    //         try {
    //             const result = await axios.get(`${REACT_APP_ENDPOINT}/tablebooking?search=${value}&page=${currentPage}&fromdate=${fromDate}&todate=${toDate}`)
                ...
                <CCol xs>
                    <CCard className="mt-4">
                        ...
                        />:'-'}</CTableDataCell> */}
                                                ...
                                                <CTableDataCell>{user.bookingId ? user.bookingId : '-'}</CTableDataCell>
                                                <CTableDataCell>{user.name ? user.name : '-'}</CTableDataCell>
                                                <CTableDataCell>{user.noOfPeople ? user.noOfPeople : '-'}</CTableDataCell>
                                                <CTableDataCell>{
                                                    user.phoneNumber == ' ' || user.phoneNumber == '' || user.phoneNumber == null ? '-' : user.phoneNumber}
                                                </CTableDataCell>
                                                <CTableDataCell>{user.time ? user.time : '-'}</CTableDataCell>
                                                <CTableDataCell>{user.date ? user.date : '-'}</CTableDataCell>
                                                <CTableDataCell>{user.status ? user.status : '-'}</CTableDataCell>
                                                <CTableDataCell>{user.outletId ? user.outletId : '-'}</CTableDataCell>

                                                <CTableDataCell>{user.created_at ? dateFormate(user.created_at) : '-'}</CTableDataCell>
                                                <CTableDataCell>{user.updated_at ? dateFormate(user.updated_at) : '-'}</CTableDataCell>
